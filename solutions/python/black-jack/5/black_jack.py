"""Functions to help play and score a game of blackjack.

How to play blackjack:    https://bicyclecards.com/how-to-play/blackjack/
"Standard" playing cards: https://en.wikipedia.org/wiki/Standard_52-card_deck
"""


def value_of_card(card):
    """Determine the scoring value of a card.

    Parameters:
        card (str): The given card.

    Returns:
        int: The value of a given card.  See below for values.

        1.  'J', 'Q', or 'K' (otherwise known as "face cards") = 10
        2.  'A' (ace card) = 1
        3.  '2' - '10' = numerical value.
    """
    if (len(card) != 1):
        return 10
    elif (ord(card) < 65):
        print("debug 0")
        if (len(card) == 1):
            return ord(card) - ord('0')
    elif (card == 'A'):
        return 1
    else:
        return 10
            
    pass


def higher_card(card_one, card_two):
    """Determine which card has a higher value in the hand.

    Parameters:
        card_one (str): First card dealt in the hand.  See below for values.
        card_two (str): Second card dealt in the hand. See below for values.

        1.  'J', 'Q', or 'K' (otherwise known as "face cards") = 10
        2.  'A' (ace card) = 1
        3.  '2' - '10' = numerical value.

    Returns:
        str or tuple: The resulting tuple contains both cards if they are of equal value.
    """
    first_value = 0
    second_value = 0
    if (len(card_one) != 1):
        first_value = 10
    if (len(card_two) != 1):
        second_value = 10
    if (len(card_one) == 1 and ord(card_one) < 65):
        first_value = ord(card_one) - ord('0')
    if (len(card_two) == 1 and ord(card_two) < 65):
        second_value = ord(card_two) - ord('0')
    if (card_one == 'A'):
        first_value = 1
    if (card_two == 'A'):
        second_value = 1
    if (len(card_one) == 1 and ord(card_one) > 65):
        first_value = 10
    if (len(card_two) == 1 and ord(card_two) > 65):
        second_value = 10

    if (first_value > second_value):
        return card_one
    elif (first_value < second_value):
        return card_two
    else:
        return card_one,card_two



def value_of_ace(card_one, card_two):
    """Calculate the most advantageous value for an upcoming ace card.

    Parameters:
        card_one (str): First card dealt in the hand.  See below for values.
        card_two (str): Second card dealt in the hand. See below for values.

        1.  'J', 'Q', or 'K' (otherwise known as "face cards") = 10
        2.  'A' (ace card) = 11 (if already in hand)
        3.  '2' - '10' = numerical value.

    Returns:
        int: Either 1 or 11, which is the value of the upcoming ace card.
    """
    print("debug 2", card_one,card_two)
    print("debug 1", value_of_card(card_one))
    if (card_one == 'A' or card_two == 'A'):
        return 1
    sum = value_of_card(card_one) + value_of_card(card_two)
    if (sum <= 10):
        return 11
    else: 
        return 1


def is_blackjack(card_one, card_two):
    """Determine if the hand is a 'natural' or 'blackjack'.

    Parameters:
        card_one (str): First card dealt in the hand.  See below for values.
        card_two (str): Second card dealt in the hand. See below for values.

        1.  'J', 'Q', or 'K' (otherwise known as "face cards") = 10
        2.  'A' (ace card) = 11 (if already in hand)
        3.  '2' - '10' = numerical value.

    Returns:
        bool: Is the hand is a blackjack (two cards worth 21).
    """
    sum1 = value_of_card(card_one) + value_of_card(card_two)
    if (card_one == 'A' or card_two == 'A'):
        sum1 += 10
    if (sum1 < 21):
        return False
    else:
        return True


def can_split_pairs(card_one, card_two):
    """Determine if a player can split their hand into two hands.

    Parameters:
        card_one (str): First card in the hand.
        card_two (str): Second card in the hand.

   Returns:
        bool: Can the hand be split into two pairs? (i.e. cards are of the same value).
    """
    sum1 = value_of_card(card_one)
    sum2 = value_of_card(card_two)

    if (sum1 == sum2):
        return True
    else:
        return False



def can_double_down(card_one, card_two):
    """Determine if a blackjack player can place a double down bet.

    Parameters:
        card_one (str): First card in the hand.
        card_two (str): Second card in the hand.

    Returns:
        bool: Can the hand can be doubled down? (i.e. totals 9, 10 or 11 points).
    """
    sum1 = value_of_card(card_one)
    sum2 = value_of_card(card_two)
    sum = sum1 + sum2 
    if (9 <= sum <= 11):
        return True
    return False
    

